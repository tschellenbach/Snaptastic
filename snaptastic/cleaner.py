...
class Cleaner(object):
    ...
    def get_snapshot_expiry_date(self, snapshot):
        ...
        if expiry_date_string:
            expiry_date = datetime.datetime.strptime(expiry_date_string, self.EXPIRY_FORMAT_STRING)
        elif expiry_delta:
    ...
    def get_expired_snapshots(self):
        ...
        print 'found %s snapshots in total, with size of %s GB' % (len(snapshots), total_size)
        print our_amis
        non_ami_snapshots = [s for s in snapshots if not any([a in s.description for a in our_amis])]
        ...
        print 'found %s rotten snapshots from %s snapshots in total' % (len(expired_snapshots), len(non_ami_snapshots))
        return expired_snapshots
