...
class Snapshotter(object):
    ...
    def __init__(self, userdata=None, metadata=None, connection=None, bdm=None):
        ...
        self.metadata = get_instance_metadata() if metadata is None else metadata
    ...
    def make_snapshot(self, vol):
        ...
        with freeze(vol.mount_point):
            snapshot = self.con.create_snapshot(volume_id, description=snapshot_name)
        #Add tags
    ...
    def mount_volume(self, ebs_volume):
        ...
        logger.info('mounting a volume to %s with snapshot %s', ebs_volume.mount_point, snapshot_id)
    ...
    def attach_volume(self, ebs_volume, boto_volume):
        ...
        logging.info('Attaching volume %s to instance %s' % (boto_volume.id, self.instance_id))
        self.con.attach_volume(boto_volume.id, self.instance_id, ebs_volume.device)
            
        # drink some coffee and wait
        while boto_volume.update() != 'in-use':
            logging.info('Waiting for volume attachment: %s' % boto_volume.id)
            sleep(1)
        while os.path.exists(ebs_volume.instance_device) == False:
    ...
    def get_bdm(self):
        bdm = self.con.get_instance_attribute(self.instance_id, 'blockDeviceMapping')
        return bdm
    ...
    def get_snapshot(self, vol):
        ...
        volume_snapshots = [s for s in all_snapshots if s.tags.get('mount_point') == vol.mount_point]
        try:
